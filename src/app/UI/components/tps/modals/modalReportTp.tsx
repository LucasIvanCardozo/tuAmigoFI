...
'use client';
import { addReportTp } from '@/app/lib/actions';
import { tps, tps_responses } from '@prisma/client';
import { useSession } from 'next-auth/react';
import { FormEvent, useState } from 'react';

export default function ModalReportTp({
  tp,
  callback,
}: {
  tp: tps;
  ...
  const formValidate = (): boolean => {
    if (!check) {
      setError('Debes estar de acuerdo el reporte a este TP');
      return false;
    }
    if (!session?.user) {
      setError('Debes iniciar sesion y reportar un TP');
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (formValidate() && session) {
      try {
        await addReportTp({ id_tp: tp.id, id_user: session.user.id });
        setConfirmed(true);
        setTimeout(() => {
          callback(undefined);
        }, 1200);
    ...
    <div className="fixed z-50 inset-0 bg-slate-800 bg-opacity-30 text-white flex justify-center items-center">
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs">
            Por favor sea reporte el TP solo si considera que este no debería
